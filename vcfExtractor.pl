...
# FIXME: tmp override for TVCv4.2.2 files as there is no version string in the VCF since this is early release
if ( grep { "##source=\"tvc .- () - Torrent Variant Caller\"" } <$vcf_fh> ) {
    print "INFO: Running on pre-release version of TVC\n";
}
elsif ( ! $tvc32 && ! grep { /^##source=(Torrent Unified Variant Caller|"tvc 4.*)/ } <$vcf_fh> ) {
...
sub filter_data {
    ...
    print "WARNING: Filtering on COSMIC ID.  This is experimental...\n\n" if ($hsids);
