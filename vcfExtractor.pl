...
#
# TODO: 
#     - Fix formatting issue.  Need to get col widths just before printing so that the output makes
#       more sense.  Right now I get col widths that might be too large for the output since there
#       is some filtering going on after the col widths are calc'd.
#
# D Sims 2/21/2013
#################################################################################################
use warnings;
...
sub format_output {
    ...
    my ($format, @header);
    if ($ovat) {
        $format = "%-19s %-${w1}s %-${w2}s %-10s %-${w3}s %-10s %-10s %-10s %-10s %-14s %-10s %-21s %s\n";
