...
sub parse_data {
    ...
    for ( @$data ) {
        ...
        # XXX
        #next unless ( $pos =~ /chr7:14045313[0-9]/ );
        #next unless ( $pos =~ /chr9/ );
        #next unless ( $pos =~ /chr12/ );

        # Grab the OVAT annotation information from the FUNC block if possible.
        my ($ovat_gc, $ovat_vc, $gene_name);
        if ( $func eq '.' ) {
            # DEBUG
            #print colored( "Got here!\n", 'bold red on_black');
            #print "$warn could not find FUNC entry for '$pos'\n";
        ...
        if ( $fro eq '.' ) {
            ...
            for my $alt_index ( 0..$#alt_array ) {
                ...
                for my $index ( @array_pos ) {
                    ...
                    # Stupid bug with de novo and hotspot merge that can create two duplicate entries for the same variant but one with and one
                    # without a HS (also different VAF, coverage,etc). Try this to capture only HS entry.
