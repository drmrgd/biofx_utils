...
if ( @cosid_lookup ) {
    #batch_lookup(\$lookup, \@cosid_lookup) if $lookup;
    if ( $lookup ) {
        print "batch cosid lookup not yet implemented\n";
        exit; 
    }
} else {
...
sub filter_data {
    ...
    if ( $fuzzy ) {
        ...
        for my $variant ( keys %$data ) {
            if (@cosid_lookup) {
                #if ( my ($query) = grep { ($_) =~ /$$data{$variant}[10]/ } @$filter ) {
    ...
    if ( $fuzzy ) {
        for my $query ( @fuzzy_pos ) {
            ...
            my $term;
            (@cosid_lookup) ? ($term = "for COSMIC ID:") : ($term = "at position:");
            for my $query ( @$filter ) {
