...
# 
# TODO:
#    - Fix the confusion OVAT / IR / Oncomine annotations options throughout script.  I think we
#      want to have -a: add IR / OVAT annots, and -O: only report Oncomine variants.
#
# D Sims 2/21/2013
#################################################################################################
use warnings;
    ...
    for ( @$data ) {
        ...
        for my $alt_index ( 0..$#alt_array ) {
            ...
            for my $index ( @array_pos ) {
                ...
                # TODO
                # This is processing even if no annots requested.  Check to see why this sub runs and why
                # We're getting unintialized variables here.  
                ...
                if ( $func eq '.' ) {
                    ...
                    ($ovat_gc, $ovat_vc, $gene_name) = get_ovat_annot(\$func, \%norm_data) unless $func eq '---'; 
                }
                ...
                push( @{$parsed_data{$var_id}}, $gene_name, $ovat_gc, $ovat_vc ) if $annots;
            }
        }
    }
    #dd \%parsed_data;
...
sub normalize_variant {
    # Borrowed from ThermoFishers vcf.py script to convert IR VCFs
    my $ref = shift;
    my $alt = shift;
    my $pos = shift;
    ...
    return ( 'nREF' => $norm_ref, 'nALT' => $norm_alt, 'nPOS' => $adj_position );
}
...
=cut
    ...
    for my $func_block ( @$json_annot ) {
        ...
        if ( $$func_block{'normalizedRef'} eq $$norm_data{'nREF'} && $$func_block{'normalizedAlt'} eq $$norm_data{'nALT'}) {
            %data = %$func_block;
        }
    }
