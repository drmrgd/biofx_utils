        ...
        for my $alt_index ( 0..$#alt_array ) {
            ...
            for my $index ( @array_pos ) {
                ...
                if ( $func eq '.' ) {
                    ...
                    ($ovat_gc, $ovat_vc, $gene_name, $hgvs, $transcript, $protein, $function, $exon) = get_ovat_annot(\$func, \%norm_data) unless $func eq '---'; 
                }
                ...
                push( @{$parsed_data{$var_id}}, $gene_name, $hgvs, $transcript, $protein, $exon, $function, $ovat_gc, $ovat_vc ) if $annots;
            }
        }
    }
    return %parsed_data;
...
sub get_ovat_annot {
    ...
    return ($gene_class, $variant_class, $gene_name, $hgvs, $transcript, $protein, $function, $exon );
}
...
sub format_output {
    ...
    if ($annots) {
        $w4 = field_width($data);
        push(@header, qw{Gene HGVS Transcript Protein Location Function oncomineGeneClass oncomineVariantClass});
        $format =~ s/\n$/ %-14s %-${w4}s %-15s %-20s %-12s %-20s %-21s %-21s\n/;
    }
    ...
    if ( $fuzzy ) {
        for my $variant ( sort { versioncmp( $a, $b ) }  keys %$data ) {
            for my $common_var ( @{$$data{$variant}} ) {
                ($nocall) ? (@output_data = @$common_var[0,1,6..18]) : (@output_data = @$common_var[0..4,6..18]);
        ...
        for my $variant ( sort { versioncmp( $a, $b ) } keys %$data ) {
            my $location;
            $$data{$variant}[15] = "Exon$$data{$variant}[15]" unless $$data{$variant}[14] eq 'intronic';
