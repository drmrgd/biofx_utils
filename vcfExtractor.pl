    ...
    for ( @$data ) {
        ...
        for my $alt_index ( 0..$#alt_array ) {
            ...
            for my $index ( @array_pos ) {
                ...
                my ($ovat_gc, $ovat_vc, $gene_name, $hgvs, $function);
                if ( $func eq '.' ) {
                    push( @warnings, "$warn could not find FUNC entry for '$pos'\n") if $annots;
                    $ovat_vc = $ovat_gc = $gene_name = "NULL";
                } else {
                    # TODO: add functional annotation here.
                    ($ovat_gc, $ovat_vc, $gene_name, $hgvs, $function) = get_ovat_annot(\$func, \%norm_data) unless $func eq '---'; 
                }
                ...
                push( @{$parsed_data{$var_id}}, $gene_name, $hgvs, $function, $ovat_gc, $ovat_vc ) if $annots;
            }
        }
    }
    #dd \%parsed_data;
...
sub get_ovat_annot {
    ...
    # Sometimes, for reasons I'm not quite sure of, there can be an array for the functional annotation.  I think it's safe to take the most severe
    # of the list and to use.  Test this out for now. This is not perfect logic, and I'm going to have to refine I'm sure!
    ...
    return ($gene_class, $variant_class, $gene_name, $hgvs, $function );
}
    ...
    if ( $fuzzy ) {
        for my $variant ( sort { versioncmp( $a, $b ) }  keys %$data ) {
            for my $common_var ( @{$$data{$variant}} ) {
                #($nocall) ? (@output_data = @$common_var[0,1,6..14]) : (@output_data = @$common_var[0..4,6..14]);
                ($nocall) ? (@output_data = @$common_var[0,1,6..15]) : (@output_data = @$common_var[0..4,6..15]);
        ...
        for my $variant ( sort { versioncmp( $a, $b ) } keys %$data ) {
            #($nocall) ? (@output_data = @{$$data{$variant}}[0,1,2,6..14]) : (@output_data = @{$$data{$variant}}[0..4,6..14]);
            ($nocall) ? (@output_data = @{$$data{$variant}}[0,1,2,6..15]) : (@output_data = @{$$data{$variant}}[0..4,6..15]);
            #dd \@output_data;
