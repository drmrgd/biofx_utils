...
sub parse_data {
    ...
    for ( @$data ) {
        ...
        #next unless $pos eq 'chr9:139390944';
...
sub get_ovat_annot {
    ...
    #($location eq 'exonic') ? ($exon = $data{'exon'}) : ($exon = 'intronic');
    ...
    return ($gene_class, $variant_class, $gene_name, $transcript, $hgvs, $protein, $function, $exon );
}
...
sub format_output {
    ...
    if ( $fuzzy ) {
        ...
        for my $variant ( sort { versioncmp( $a, $b ) } keys %$data ) {
            my $location = $$data{$variant}[15];
            #$$data{$variant}[15] = "Exon$location" if $location =~ /\d+/;
            #$$data{$variant}[15] = "Exon$$data{$variant}[15]" unless $$data{$variant}[15] eq 'intronic' or $$data{$variant}[15] eq '---';
            $$data{$variant}[15] = "Exon$$data{$variant}[15]" if int($$data{$variant}[15]);
