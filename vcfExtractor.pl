...
sub get_ovat_annot {
    ...
    no warnings;
    my $func = shift; # FUNC Block from VCF Entry
    my $norm_data = shift; # Normalized REF ALT and POS.
    $$func =~ tr/'/"/;
    ...
    # TODO: remove me
    #return unless grep{ $$_{'gene'} eq 'TP53' } @$json_annot;
    ...
    for my $func_block ( @$json_annot ) {
        ...
        if ($$func_block{'normalizedRef'}) {
            ...
            #else {
                #$data{'gene'} = $$func_block{'gene'};
                #$data{'location'} = $$func_block{'location'};
            #}
        } 
        #else {
            #%data = %{$$json_annot[0]};
    ...
    #%data = %$norm_data if $match == 0;

    
    #my $count = grep{$data{$_} eq '---'} keys %data;
    #if ($count > 8) {
    #if (! $data{'location'} ) {
        #print colored("COUNT > 12:  $count\n", 'bold red on_black');
        #dd $json_annot;
        #dd %data;
    #}
