    ...
    for ( @$data ) {
        ...
        for my $alt_index ( 0..$#alt_array ) {
            ...
            for my $index ( @array_pos ) {
                ...
                my ($ovat_gc, $ovat_vc, $gene_name, $hgvs, $function, $exon);
                if ( $func eq '.' ) {
                    push( @warnings, "$warn could not find FUNC entry for '$pos'\n") if $annots;
                    $ovat_vc = $ovat_gc = $gene_name = "NULL";
                } else {
                    # XXX
                    ($ovat_gc, $ovat_vc, $gene_name, $hgvs, $function, $exon) = get_ovat_annot(\$func, \%norm_data) unless $func eq '---'; 
                }
                ...
                push( @{$parsed_data{$var_id}}, $gene_name, $hgvs, $exon, $function, $ovat_gc, $ovat_vc ) if $annots;
            }
        }
    }
    return %parsed_data;
...
sub get_ovat_annot {
    ...
    return ($gene_class, $variant_class, $gene_name, $hgvs, $function, $exon );
}
...
sub ovat_filter {
    ...
    if ( $ovat_filter ) {
        ...
        for my $variant ( keys %$data ) {
            delete $$data{$variant} if $$data{$variant}->[16] eq '---';
        }
    }
    format_output($data);
...
sub format_output {
    ...
    if ($annots) {
        $w4 = field_width($data);
        push(@header, qw{Gene HGVS Location Function oncomineGeneClass oncomineVariantClass});
        $format =~ s/\n$/ %-14s %-${w4}s %-12s %-20s %-21s %-21s\n/;
    }
    ...
    if ( $fuzzy ) {
        ...
        for my $variant ( sort { versioncmp( $a, $b ) } keys %$data ) {
            my $location;
            $$data{$variant}[13] = "Exon$$data{$variant}[13]" unless $$data{$variant}[13] eq 'intronic';
            ($nocall) ? (@output_data = @{$$data{$variant}}[0,1,2,6..16]) : (@output_data = @{$$data{$variant}}[0..4,6..16]);
            printf {$out_fh} $format, @output_data;
        }
    }
