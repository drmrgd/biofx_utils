...
die "ERROR: OVAT output selected, but no OVAT annotations found in VCF file!\n" if ( $ovat && $ovat_annot == 0 ); 
close $vcf_fh;
...
print "OVAT filter status: $foo\n";

# Filter and format extracted data or just format and print it out.
if ( @cosids ) {
    filter_data(\%vcf_data, \@cosids, $ovat_filter);
}
elsif ( @coords ) {
    filter_data(\%vcf_data, \@coords, $ovat_filter);
} 
else {
    format_output(\%vcf_data, $ovat_filter);
}
...
sub parse_data {
    ...
    for ( @$data ) {
        ...
        #print "func_block: $func\n";
        #exit;

        my ( $ovat_gc, $ovat_vc );
        ($func eq '.') ? next : (($ovat_gc, $ovat_vc) = get_ovat_annot( \$func ) );
        #get_ovat_annot( \$func ) unless $func eq '.';
        #print "oncomineGeneClass    => $ovat_gc\n";
        #print "oncomineVariantClass => $ovat_vc\n";
        #next;
        #exit;
...
sub filter_data {
    ...
    my $data = shift;
    my $filter = shift;
    my $ovat_filter = shift; # only report oncomine annotated vars.
    ...
    if ( $fuzzy ) {
        ...
        for my $variant ( keys %$data ) {
            next if ( $ovat_filter && $$data{$variant}[11] eq '---' );
            if ($hsids) {
    ...
    # TODO: Fix up logic here.  If there is a missing variant due to a missing ovat annot,then it reports that no variant was 
    #       found at the position, not indicating that it was filtered since it didn't have an ovat annot (there actually is a
    #       variant at the position and it was filtered due to ovat).
    if ( $ovat_filter && ! %filtered_data ) {
...
sub format_output {
    ...
    my ($format, @header);
    if ($ovat) {
        $format = "%-19s %-${w1}s %-${w2}s %-10s %-${w3}s %-10s %-10s %-10s %-10s %-14s %-21s %s\n";
        @header = qw( CHROM:POS REF ALT Filter Filter_Reason VAF TotCov RefCov AltCov COSID oncomineGeneClass oncomineVariantClass );
    } else {
    ...
    if ( $fuzzy ) {
        for my $variant ( sort { versioncmp( $a, $b ) }  keys %$data ) {
            for my $common_var ( @{$$data{$variant}} ) {
                if ($ovat_filter) { next unless $$common_var[11] eq '---' }
                printf $out_fh $format, @$common_var[0..4,6..12];
            }
        }
    } else {
        for my $variant ( sort { versioncmp( $a, $b ) } keys %$data ) {
            if ($ovat_filter) { next if $$data{$variant}[11] eq '---' }
            printf $out_fh $format, @{$$data{$variant}}[0..4,6..12];
        }
    }
}
...
sub get_ovat_annot {
    ...
    return ($gene_class, $variant_class);
}
